...
(subtest "Sinks tests"
  (subtest "Single-thread sink"
    (let ((sink (make-instance 'single-thread-sink)))
      ...
      (is (length (sink-handlers-list sink)) 1000)
      (is (find 'handler54 (sink-handlers-list sink)) 'handler54)
      (remove-event-handler sink 'handler54)
      (is (length (sink-handlers-list sink)) 999)
      (is (find 'handler54 (sink-handlers-list sink)) nil)))
